...
import type { DocumentPath } from 'mongolike-operations'
...
export function getNestedValue(
  ...
  for (const key of keys) {
    try {
      ...
      if (noNullAccess) {
        throw new Error(
          `Cannot get value by the document path: "${docPath}". The property "${
            key
          }" is not found in the upper undefined object.`
        )
      }
      return undefined
    }
