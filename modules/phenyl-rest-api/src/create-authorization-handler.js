...
function assertAuthorizationFunction(
  fn: any,
  name: string,
  methodName: string
) {
  if (typeof fn !== 'function')
    throw new Error(
      `No authorization function found for ${name} (methodName = ${methodName})`
    )
}
  ...
  return async function authorizationHandler(
    ...
    switch (reqData.method) {
      ...
      case 'delete': {
        ...
        assertAuthorizationFunction(
          entityDefinition.authorization,
          data.entityName,
          method
        )
      ...
      case 'runCustomQuery': {
        ...
        if (customQueryDefinition == null)
          throw new Error(`Unknown custom query name "${payload.name}".`)
        assertAuthorizationFunction(
          customQueryDefinition.authorization,
          payload.name,
          method
        )
      ...
      case 'runCustomCommand': {
        ...
        if (customCommandDefinition == null)
          throw new Error(`Unknown custom command name "${payload.name}".`)
        assertAuthorizationFunction(
          customCommandDefinition.authorization,
          payload.name,
          method
        )
      ...
      case 'login': {
        ...
        if (userEntityDefinition == null)
          throw new Error(`Unknown entity name "${payload.entityName}".`)
        assertAuthorizationFunction(
          userEntityDefinition.authorization,
          payload.entityName,
          method
        )
