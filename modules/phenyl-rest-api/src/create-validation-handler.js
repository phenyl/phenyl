...
function assertValidationFunction(fn: any, name: string, methodName: string) {
  if (typeof fn !== 'function')
    throw new Error(
      `No validation function found for ${name} (methodName = ${methodName})`
    )
}
  ...
  return async function validationHandler(
    ...
    switch (reqData.method) {
      ...
      case 'runCustomQuery': {
        ...
        assertValidationFunction(
          customQueryDefinition.validation,
          payload.name,
          method
        )
      ...
      case 'runCustomCommand': {
        ...
        assertValidationFunction(
          customCommandDefinition.validation,
          payload.name,
          method
        )
      ...
      case 'logout': {
        ...
        assertValidationFunction(
          userEntityDefinition.validation,
          payload.entityName,
          method
        )
