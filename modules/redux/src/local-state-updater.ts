...
export class LocalStateUpdater<TM extends TypeMap> {
  // static LocalStateFinder: Class<LocalStateFinder<TM>> = LocalStateFinder

  /**
   * Initialize the given entity field.
   */
  static initialize<N extends EntityNameOf<TM>>(state: LocalStateOf<TM>, entityName: N): UpdateOperation {
    return {
      $set: {
  ...
  static commit<N extends EntityNameOf<TM>>(state: LocalStateOf<TM>, command: IdUpdateCommand<N>): UpdateOperation {
  ...
  static revert<N extends EntityNameOf<TM>>(state: LocalStateOf<TM>, command: RevertCommand<N>): UpdateOperation {
  ...
  static unfollow<N extends EntityNameOf<TM>>(state: LocalStateOf<TM>, entityName: N, id: Id): UpdateOperation {
    return {
      $unset: {
  ...
  static removeNetworkRequest(state: LocalStateOf<TM>, tag: ActionTag): UpdateOperation {
    return {
      $set: {
        [createDocumentPath('network', 'requests')]: removeOne(state.network.requests, tag),
      },
    }
  }
  /**
  ...
  static rebase<N extends EntityNameOf<TM>>(state: LocalStateOf<TM>, pushCommand: PushCommand<N>): UpdateOperation {
  ...
  static synchronize<N extends EntityNameOf<TM>>(
    ...
    const newOrigin = assign(entityInfo.origin, ...operations, ...localCommits) // assert(localCommits.length === 0 || entityInfo.commits[0] === localCommits[0])
