...
export function assertValidPullQuery(q: any): void {
  ...
  assertStringOrNull(
    versionId,
    `PullQuery.versionId must be a string or null. "${versionId}" given.`
  );
}
/**
 *
...
export function assertValidDeleteCommand(com: any): void {
  ...
  throw new Error(
    'DeleteCommand must have key "id" or "where". Neither given.'
  );
}
/**
 *
...
export function assertValidCustomQuery(q: any): void {
  ...
  const { name, params } = q;
  assertValidCustomName(name, "CustomQuery"); // params can be null
...
export function assertValidCustomCommand(com: any): void {
  ...
  assertValidCustomName(name, "CustomCommand"); // params can be null
...
export function assertValidLoginCommand(com: any): void {
  ...
  assertValidEntityName(entityName, "LoginCommand"); // credentials must be an object
...
export function assertValidEntityName(entityName: any, _dataName?: string) {
  ...
  if (!/[A-Za-z][A-Za-z0-9-_]*/.test(entityName)) {
    throw new Error(
      `${dataName}entityName must be the regex: "[A-Za-z][A-Za-z0-9-_]*". "${entityName}" given.`
    );
  }
}
/**
...
export function assertValidCustomName(name: any, _dataName?: string) {
  ...
  if (!/[A-Za-z][A-Za-z0-9-_]*/.test(name)) {
    throw new Error(
      `${dataName}name must be the regex: "[A-Za-z][A-Za-z0-9-_]*". "${name}" given.`
    );
  }
}
/**
